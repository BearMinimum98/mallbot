...
s = Session()
searchTerms = []
if isinstance(formData['booze'], list):
	searchTerms = formData.getvalue('booze')
else:
	searchTerms = [formData.getvalue('booze')]
# add the ingredients as an array to the dictionary term
recipe = {'parisian cathouse': ['raspberry', 'boxed champagne'], 'prussian cathouse': ['parisian cathouse', 'magical ice cubes'], 'vodka martini': ['olive', 'bottle of vodka'], 'rockin\' wagon': ['vodka martini', 'magical ice cubes'], 'soft green echo eyedrop antidote martini': ['rockin\' wagon', 'soft green echo eyedrop antidote'], 'sangria de menthe': ['peppermint twist', 'boxed wine'], 'fuzzbump': ['whiskey sour', 'coconut shell'], 'whiskey sour': ['lemon', 'bottle of whiskey'], 'ducha de oro': ['tequila sunrise', 'little paper umbrella'], 'tequila sunrise': ['orange', 'bottle of tequila'], 'tequila sunset': ['tangerine', 'bottle of jorge sinsonte'], "dry vodka martini": ["jumbo olive", "bottle of vodka"]}
# meat you are willing to spend in on each type booze
meatLimit = int(formData.getvalue('meat'))
# how much you want to undercut the lowest price on the market
underCut = int(formData.getvalue('undercut'))
# login with your username as password here
try:
	s.login(formData.getvalue('username'), formData.getvalue("password"))
except:
	print "Login error"
# end options
myStoreId = s.userId
